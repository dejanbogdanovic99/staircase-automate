...
class MovingTimeTests : public MovingInitializationTests {
  public:
    MovingTimeTests()
        : MovingInitializationTests{},
          mMoving{mBasicLightRefs, staircase::Moving::Direction::UP, 12000} {}
...
TEST_F(MovingTimeTests,
       GivenUpMovingIsNotFinishedCompletedReturnsFalse) {
    mMoving.update(50);
...
TEST_F(MovingTimeTests,
       GivenUpMovingIsFinishedCompletedReturnsTrue) {
...
TEST_F(MovingUpLightTests,
    ...
    for (std::size_t i = 1; i < mBasicLights.size(); ++i) {
        EXPECT_CALL(mBasicLights[i], turnOn(staircase::IBasicLight::kDefaultOnPeriod))
            .Times(Exactly(1));
        if (i < 3)
        {
            mMoving.update(500);
        ...
        {
            mMoving.update(12000 / mBasicLights.size());
        }
    }
}
...
class MovingDownLightTests : public MovingInitializationTests {
  public:
    MovingDownLightTests()
        : MovingInitializationTests{},
          mMoving{mBasicLightRefs, staircase::Moving::Direction::DOWN, 12000} {}
...
TEST_F(MovingDownLightTests,
    ...
    mMoving.update(50);
    for (std::size_t i = 1; i < mBasicLights.size(); ++i) {
        EXPECT_CALL(mBasicLights[staircase::IBasicLight::kLightsNum - i - 1], turnOn(staircase::IBasicLight::kDefaultOnPeriod))
            .Times(Exactly(1));
        if (i < 3)
        {
            mMoving.update(500);
